  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl px-4 py-20 md:px-8 lg:px-10">
        <h1 className="mb-4 max-w-4xl text-3xl font-bold tracking-tight text-foreground md:text-5xl">
          Professional Experience
        </h1>
        <p className="max-w-2xl text-base text-foreground/70 md:text-xl">
          Impact by role using Challenge → Action → Result framework across
          my journey from SDE III to Full-Stack Development and Training.
        </p>
      </div>
          ...
          >
            <div className="sticky top-40 z-40 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm">
              <div className="absolute left-3 flex h-10 w-10 items-center justify-center rounded-full bg-background md:left-3">
                <div className="h-4 w-4 rounded-full border border-border bg-foreground p-2" />
              </div>
              <h3 className="hidden text-xl font-bold text-foreground md:block md:pl-20 md:text-5xl">
                {item.title}
              </h3>
            </div>
